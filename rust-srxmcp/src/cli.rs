...
pub struct Cli {
    ...
    #[arg(
        long,
        env = "JMCP_SRX_MAX_REQUESTS_PER_SECOND_PER_TOKEN",
        default_value_t = 0
    )]
    ...
    #[arg(
        long,
        env = "JMCP_SRX_MAX_REQUEST_BURST_PER_TOKEN",
        default_value_t = 0
    )]
    ...
    #[arg(long, env = "JMCP_SRX_MAX_SESSIONS_PER_TOKEN", default_value_t = 16)]
...
mod tests {
    ...
    fn secure_defaults() {
        ...
        let custom = Cli::parse_from(["rust-srxmcp", "--max-sessions-per-token", "9"]);
