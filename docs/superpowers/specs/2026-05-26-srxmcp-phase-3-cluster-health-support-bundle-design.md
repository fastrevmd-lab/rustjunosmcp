...
| `vpn` | `get-ike-security-associations`, `get-ike-active-peer-information`, `get-ipsec-security-associations`, `get-ipsec-statistics-information`, `get-security-associations-information` | `/var/log/kmd` | RPC names verified during capture pass before code lands. `ipsec-key-management` rolled into `kmd` on 24.4R1. |
| `traffic_loss` | `get-flow-session-summary-information`, `get-flow-session-information`, `get-interface-information`, `get-policy-hit-count` | flowd traces if enabled | |
| `idp_appid` | `get-idp-policy-version`, `get-appid-status`, `get-idp-security-package-version` | `/var/log/idpd`, `/var/log/appid` | Reuse Phase 1B + Phase 2 RPCs |
| `routing` | `get-route-summary-information`, `get-bgp-summary-information`, `get-ospf-neighbor-information` | `/var/log/rpd` | |
...
**Verification gate:** every RPC name in the table above is treated as *unverified* until live-captured against the lab during the v0.3.0 implementation pass. Names follow the Junos `get-<noun>-information` convention but Phase 2 already showed (`request-appid-application-package-*` vs the design-doc guess) that CLI namespace ≠ RPC name. Capture before code.
...
4. **Pre-implementation capture pass** — verify the `vpn`, `traffic_loss`, `routing`, and `idp_appid` per-`problem_type` RPC names against the lab (Phase 2 showed CLI namespace ≠ RPC name). Capture before code.
