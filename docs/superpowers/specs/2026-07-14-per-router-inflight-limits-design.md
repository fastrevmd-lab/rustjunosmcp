...
The implementation adds `serde_json` and `http-body-util` as direct dependencies
of `rust-junosmcp-limits` from the existing resolved workspace dependency set.
`http-body-util 0.1.3` was already transitively locked and is named directly only
to identify `LengthLimitError`; no package version, source, checksum, or new
external runtime package is introduced. `rust-junosmcp-core` and `tempfile` are
test-only dependencies for lease-composition coverage. Any `Cargo.lock` change
is generated by Cargo and limited to recording the workspace crate's new direct
dependencies.
