...
**Risk note for implementer:** (1) `apply_pipe_modifiers` splits on `|`; a command whose *content* contains a literal `|` (rare in Junos op commands) would be mis-parsed — acceptable, but if a segment isn't `count`/`last N` it's simply skipped, so worst case is a no-op. (2) The byte-cap marker adds bytes beyond `max_bytes`; that's intentional (marker is meta). (3) Keep each `server.rs` tool description a single string literal — don't break it across concatenated literals in a way that changes the advertised text.
