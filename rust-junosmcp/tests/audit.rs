...
fn add_device_audit_omits_credentials() {
    ...
    let out = run_with_capture(|| {
        let mut a = AuditScope::stdio("add_device", "add-device", vec![]);
        a.meta("name", "r99");
...
fn commit_audit_logs_hash_not_body() {
    let out = run_with_capture(|| {
        let mut a = AuditScope::stdio("load_and_commit_config", "commit", vec!["r1".into()]);
...
fn execute_command_redacts_command_output() {
    let out = run_with_capture(|| {
        let mut a = AuditScope::stdio("execute_junos_command", "execute", vec!["r1".into()]);
...
fn transfer_file_logs_sha256_not_content() {
    let out = run_with_capture(|| {
        let mut a = AuditScope::stdio("transfer_file", "transfer", vec!["r1".into()]);
...
fn audit_scope_emits_unsettled_on_drop() {
    ...
    let out = run_with_capture(|| {
        let _a = AuditScope::stdio("upgrade_junos", "upgrade", vec!["r1".into()]);
...
fn batch_command_count_logged() {
    ...
    assert!(out.contains("devices=r1,r2"), "output: {}", out);
}

#[test]
fn template_var_count_logged() {
    let out = run_with_capture(|| {
        let mut a = AuditScope::stdio("render_and_apply_j2_template", "apply", vec!["r1".into()]);
